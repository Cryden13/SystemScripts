...
class Options:
    ...
    add_params: str = CONV_ADD_PARAMS

    @classmethod
    def getInput(cls, fpath: Path) -> bool:
        fields = [
            ("Convert video", InDlg.ChWgt.checkbox(default=cls.conv_vid)),
            ("Convert audio", InDlg.ChWgt.checkbox(default=cls.conv_aud)),
            ("Convert subtitles", InDlg.ChWgt.checkbox(default=cls.conv_sub)),
            (
                "Audio stream",
                InDlg.ChWgt.combobox(
                    options=[
                        "all",
                        *list(CONV_AUD_LANGS),
                        *[str(i) for i in range(CONV_STREAMS)],
                    ],
        ...
        if fpath.is_dir():
            ...
            fields.insert(
                ...
                (
                    "Output format",
                    InDlg.ChWgt.combobox(
                        options=["auto", ".mkv", ".mp4", ".webm"], default=".mkv"
                    ),
                ),
            )
            fields += [
                ...
                (
                    "Webm cutoff (sec)",
                    InDlg.ChWgt.spinbox(from_=0, to=3600, default=cls.playtime),
                ),
                (
                    "Thread count",
                    InDlg.ChWgt.spinbox(
                        from_=1, to=(3 if CONV_NVENC else 20), default=cls.thread_ct
                    ),
                ),
            ]
            msg = (
        ...
        fields.append(
            ("Additional arguments", InDlg.ChWgt.textbox(default=cls.add_params))
        )
        ...
        if ans:
            cls.conv_vid = ans["Convert video"]
            cls.conv_aud = ans["Convert audio"]
            cls.conv_sub = ans["Convert subtitles"]
    ...
    def getInfo(cls) -> list[str]:
        ...
        if not cls.conv_sub:
            out.insert(0, "Convert subtitles: NO")
        if not cls.conv_aud:
            out.insert(0, "Convert audio: NO")
        if not cls.conv_vid:
            out.insert(0, "Convert video: NO")
        if cls.isdir:
