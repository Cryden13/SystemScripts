...
class ConvertVideo:
    ...
    def run(self, fpath: Path):
        ...
        prev_parent: Path = None
        for i, pth in enumerate(self.files):
            progress.setValue(i)
            if progress.wasCanceled():
                progress.setValue(self.totct)
                raise SystemExit
            if not prev_parent or not pth.parent.samefile(prev_parent):
                prev_parent = pth.parent
                options = Opt()
            data.append(dict(pth_in=pth, info=BuildCmd(pth, options)))
        progress.setValue(self.totct)
        # run
    ...
    def process(self, pth_in: Path, info: BuildCmd) -> None:
        ...
        err = None
        proc_dict = dict(V=dict(c='converted', x='cropped'),
