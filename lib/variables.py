...
class _CFG(_ConfigParser):
    def __init__(self):
        _ConfigParser.__init__(self, allow_no_value=False, interpolation=_ExtInterp())
...
CONV_CRF_VALS = tuple(
    (int(ht), int(crf))
    for ht, crf in [
        _split(r"\s*:\s*", val) for val in _cfg.getlines(_sct, "ffmpeg_crf_values")
    ]
)
CONV_AUD_LANGS = {
    x[0]: x[1]
    for x in [line.split(":") for line in _cfg.getlines(_sct, "aud_stream_langs")]
}
CONV_SUB_LANGS = {
    x[0]: x[1]
    for x in [line.split(":") for line in _cfg.getlines(_sct, "sub_stream_langs")]
}
...
CONV_ADD_PARAMS = _cfg.get(_sct, "add_params").strip()
